// LoadTest.cpp
// Attempt to pin down deserialization bug
...
#include "../../SurfaceReader/Protocol.h"
...
boost::filesystem::path myFile( "\\Users\\tim\\projects\\BoostSerializationIssue\\Debug\\Tranzport.spu");
		...
		boost::filesystem::path ProcessingPath = myFile;
						boost::filesystem::wifstream WInputStream(ProcessingPath.generic_wstring());
// Commenting out this line stops the crash when the main function returns		
						// boost::archive::xml_wiarchive archive(WInputStream);

		try
		{
			// Having this line active or commented out makes no difference
			// archive >> BOOST_SERIALIZATION_NVP(myProtocol);
			WInputStream.close();
		}
				catch (...)
		{
